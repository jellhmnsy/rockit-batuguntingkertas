...
const ThemeProvider = ({ children }) => {
  ...
  //Sync state dengan local storage jka ada perubahan
  useEffect(() => {
    localStorage.setItem('themeApp', JSON.stringify(isDark));
  }, [isDark]); // Dependent, kalau ada perubahan di state isDark, make function ini dijalankan
    ...
    <ThemeProvider>
      <div className="max-w-md mx-auto p-4 space-y-4">
        <div className="flex justify-between items-center">
          <h1 className="text-xl font-bold">Theme Context Demo Local Storage</h1>
          <ThemeToggle />
        </div>
        <Card />
      </div>
